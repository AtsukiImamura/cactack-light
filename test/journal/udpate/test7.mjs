...
const TEST_NAME = "journal__update_test_7"
/**
 * --------------------------------------------------------------------------------------
 * ## test ##
...
export async function test(context) {
    ...
    const categoryItems = await commonUtil.prepareItems(context.user.uid, 4, TEST_NAME)

    const item1 = categoryItems[0]
    const item2 = categoryItems[1]
    const item3 = categoryItems[2]
    const item4 = categoryItems[3]
...
export async function result(context) {
  ...
  //   assert.strictEqual(ledger.data[context.item3.id], 0)
  //   assert.strictEqual(ledger.data[context.item4.id], 0)
  // }
  {
    const dataRef = (await admin.firestore().collection("ledger")
      .doc(`${context.user.uid}.2020.12`).get())
    assert.strictEqual(dataRef.exists, true)
    const ledger = dataRef.data()
    assert.strictEqual(ledger.data[context.item3.id], 400)
    assert.strictEqual(ledger.data[context.item4.id], -400)
  }
}