...
const TEST_NAME = "journal__update_test_2"
/**
 * --------------------------------------------------------------------------------------
 * ## test ##
 * 複数仕訳があり、過去のものを変更した際にそれ以降の日付の履歴が更新されること（異なる月での変更）
 * 
 * --------------------------------------------------------------------------------------
 */
...
export async function result(context) {
  {
    ...
    assert.strictEqual(histoies[0].amount,450)
  ...
  }
  {
    ...
    assert.strictEqual(balance.data[context.item1.id], 450)
    assert.strictEqual(balance.data[context.item2.id], -450)
  }
  {
    const dataRef = (await admin.firestore().collection("balance")
  ...
  }
  {
    ...
    assert.strictEqual(ledger.data[context.item1.id], 450)
    assert.strictEqual(ledger.data[context.item2.id], -450)
  }
  {
    const dataRef = (await admin.firestore().collection("ledger")
      .doc(`${context.user.uid}.2020.12`).get())
    assert.strictEqual(dataRef.exists, true)
    const ledger = dataRef.data()
    assert.strictEqual(ledger.data[context.item1.id], 200)
    assert.strictEqual(ledger.data[context.item2.id], -200)
  }
}