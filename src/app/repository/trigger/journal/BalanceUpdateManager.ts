...
import BalanceFirestore from "../../firestore/BalanceFirestore";
...
export default class BalanceUpdateManager extends UpdateManagerBase<DBalance>{
    ...
    protected async loadTargets() {
        ...
        const balances = (await container.resolve(BalanceFirestore).search(q => 
    ...
    private async createNewBalance(dateKeyInfo: {year: number, month: number, key: string}) {
        const pastBalances = (await container.resolve(BalanceFirestore).search(q =>    
            q.where("userId", "==", this.after.userId)
        ...
        await container.resolve(BalanceFirestore).setById(targetBalance.id,targetBalance)
        return targetBalance
    }

    protected async commit() {
        // console.log("@balance commit")
        // console.log(this.targets)
        await Promise.all(this.targets.map(bls => container.resolve(BalanceFirestore).update(bls.id, bls)))
    }

    protected async update(journal: DJournal, acc: boolean) {
        const dateInfo = this.createDateInfo(journal.accountAt)
        // console.log(`@balance update: ${acc} ${dateInfo.key}`)
        // console.log(this.targets)
        for(const bls of this.targets){
            ...
            const updateBalance = (details: DJournalDetail[], dAcc:boolean) =>{
                ...
                // console.log("@update balance", this.targets)
            }
